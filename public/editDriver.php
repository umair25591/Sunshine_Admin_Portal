...
if(isset($_SESSION['adminId'])){
    ...
    if(isset($_GET['driverId'])){
        ...
        $getDriverData = "SELECT * FROM drivers WHERE id = {$driverId}";
        $runDriverData = mysqli_query($conn, $getDriverData);
        $numRowDriverData = mysqli_num_rows($runDriverData);

        if($numRowDriverData > 0){
            $resultDriverData = mysqli_fetch_assoc($runDriverData);
            $selectedEmployeeType = $resultDriverData['employeeType'];
            $selectedPayStructure = $resultDriverData['payStructure'];
        }
    }
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row mb-24 gy-3 align-items-center">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="firstName" id="firstName" class="form-control" placeholder="Enter First Name" value="<?php echo $resultDriverData['firstName'] ?>">
                                </div>
                            </div>
                            <div class="row mb-24 gy-3 align-items-center">
                                <label class="form-label mb-0 col-sm-2">Last Name</label>
                                <div class="col-sm-10">
                                    <input type="text" name="lastName" id="lastName" class="form-control" placeholder="Enter Last Name" value="<?php echo $resultDriverData['lastName'] ?>">
                                </div>
                            </div>
                            <div class="row mb-24 gy-3 align-items-center">
                                <label class="form-label mb-0 col-sm-2">Mobile</label>
                                <div class="col-sm-10">
                                    <input type="number" name="mobile" id="mobile" class="form-control" placeholder="Enter Mobile" value="<?php echo $resultDriverData['mobile'] ?>">
                                </div>
                            </div>
                            <div class="row mb-24 gy-3 align-items-center">
                                ...
                                <?php
                                    if ($numRowsEmplyeeType > 0) {
                                        while ($resultEmployeeType = mysqli_fetch_assoc($runGetEmplyeeType)) {
                                            $checked = ($resultEmployeeType['id'] == $selectedEmployeeType) ? 'checked' : '';
                                            echo '<div class="bg-primary-50 px-20 py-12 radius-8">
                                                    <span class="form-check checked-primary d-flex align-items-center gap-2">
                                                        <input class="form-check-input" type="radio" name="employeeType" id="employeeType' . $resultEmployeeType['id'] . '" value="' . $resultEmployeeType['id'] . '" ' . $checked . '>
                                ...
                                <?php
                                    if ($numRowPayStructure > 0) {
                                        while ($resultPayStructure = mysqli_fetch_assoc($runPayStructure)) {
                                            $checked = ($resultPayStructure['id'] == $selectedPayStructure) ? 'checked' : '';
                                            echo '<div class="bg-primary-50 px-20 py-12 radius-8">
                                                    <span class="form-check checked-primary d-flex align-items-center gap-2">
                                                        <input class="form-check-input" type="radio" name="payStructure" id="payStructure' . $resultPayStructure['id'] . '" value="' . $resultPayStructure['id'] . '" ' . $checked . '>
                            ...
                            <div class="row mb-24 gy-3 align-items-center">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="uniqueId" id="uniqueId" class="form-control" placeholder="Enter Unique ID"  value="<?php echo $resultDriverData['keyPayUniqueID'] ?>">
                                </div>
                            </div>
                            <div class="row mb-24 gy-3 align-items-center">
                                <label class="form-label mb-0 col-sm-2">Change Pin</label>
                                <div class="col-sm-10">
...
<script src="assets/js/editDriver.js"></script>